...
Tab 3: buy-side layer + live chat. Static section: 5 pre-run buy-side Q&As (accordion) generated by `demo/data/analysis/run_buyside_analysis.py`. Chat section: Claude Opus + Tavily web search via `demo/server.py` (FastAPI, SSE streaming, DOMPurify). Start: `python3 demo/server.py` → `http://localhost:8000`. Server pings on tab load and shows live/offline badge. Suggestion chips pre-fill common questions. Clear history button resets chat. Agentic loop handles compound questions: Claude may issue multiple parallel searches per turn; all tool_use blocks are matched with tool_results before the next round.
...
- [x] Buy-side analysis script: `demo/data/analysis/run_buyside_analysis.py` — 5 questions, claude-opus-4-7
- [x] Output: `panw_q2fy26_buyside_analysis.json` — 5 substantive answers (Q1–Q5)
- [x] Server: `demo/server.py` — FastAPI, SSE streaming, Tavily tool use, CORS
- [x] Tab 3 HTML: framing card, 5 accordion cards, chat section, "What's powering this" sauce panel
...
- [x] End-to-end tested: question → Claude streams response from earnings context
