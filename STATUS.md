...
Tab 3: buy-side layer + live chat. Static section: 5 pre-run buy-side Q&As (accordion) generated by `demo/data/analysis/run_buyside_analysis.py`. Chat section: Claude Opus + Tavily web search via `demo/server.py` (FastAPI, SSE streaming, DOMPurify). Start: `python3 demo/server.py` → `http://localhost:8000`. Server pings on tab load and shows live/offline badge. Suggestion chips pre-fill common questions.
