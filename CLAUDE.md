...
**For the analytical demo design (post pipeline rebuild):** read this file, then `demo/demo_approach.md`, then `demo/earnings_analysis_script.md` (the complete Cowork briefing — data package, analytical framework, prompts, and guardrails).
...
**Build status:** Replaced by Microsoft Forms + GitHub Pages. The custom feed app was not built. Microsoft Forms handles the Beat 3 post (B/H/S + conviction + uncertainty) and settling poll. Results are projected from the Forms presenter view. The `feed-app/` subfolder is a dead artifact — retire or delete it.
...
**Resolved 2026-06-03:** Demo is live in Cowork, two moves. Move 1 (~10 min): sell-side foundation using the structured data package in `demo/earnings_analysis_script.md` (Steps 5–8). Move 2 (~3 min): Decision Layer using the 5-dimension buy-side framework. Fallback materials are a remaining open item.
...
Phase 2 (demo build) is **complete and workshop-ready as of 2026-06-03**. The data pipeline rebuild landed in late May; the Q3 FY26 pipeline run (gather → rebuild_db → analysis scripts → generate_baseline) executed cleanly on June 3 after the PANW Q3 FY26 print (June 2). All three dashboard tabs render from current Q3 FY26 data: Tab 1 (baseline data + sentiment captured via Playwright), Tab 2 ("Earnings Reviewer" — sell-side skill applied with Rating Logic and Margin Analysis sections showing the skill's reasoning chain; rating is Maintain Outperform with PT $174), Tab 3 ("Decision Layer" — buy-side judgment overlay with 5-dimension framework and live chat via Claude + Tavily). The presenter script `demo/earnings_analysis_script.md` is refreshed to Q3 FY26 actuals and now includes a Cowork-facing orientation block (tells Claude what to do when the document is dropped into a session) and Appendix C (seven analyst guardrails covering the Q3-specific analytical traps). `demo/EARNINGS-ANALYSIS-GUIDE.md` has been retired — its content was Q2 FY26 and is superseded by the script. Server smoke tests pass end-to-end. The mobile-first PANW one-pager for phone participants is live on GitHub Pages at https://sandeepagi.github.io/earnings-demo/ (generated by `demo/generate_one_pager.py` → `docs/index.html`; facts only — no rating/PT/bull-bear framing — with tap-to-copy per section); its QR code is in deck Slide 7. Remaining for workshop day: fallback materials (mandatory per hard rules) and the workshop facilitation deliverables noted above.
