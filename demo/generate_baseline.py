...
Reads: /tmp/earnings_v2.db  (built by demo/data/rebuild_db.py)
Writes: demo/earnings_baseline.html  (to mounted workspace)

Three-tab output:
  Tab 1 — Baseline Data:      data tables, KPIs, charts from the DB
  Tab 2 — Claude Analysis:    Steps 5–8 framework, generated by Claude in Cowork
                               from structured DB data. NOT plugin output — the
                               Earnings Reviewer plugin is not installed.
  Tab 3 — Buy-Side Layer:     Infrastructure cards (what we built) + 4 prompt cards
                               for live Cowork demo. No pre-written conclusions.
...
DB_PATH       = '/tmp/earnings_v2.db'
OUT_PATH      = '/sessions/trusting-brave-ritchie/mnt/earnings-demo/demo/earnings_baseline.html'
CHARTJS_CACHE = '/tmp/chartjs_440.min.js'
...
insiders      = q("SELECT * FROM insider_transactions WHERE symbol=? ORDER BY transaction_date", (PRIMARY_SYMBOL,))
...
ah_chg      = kpis.get('stock_ah_change_pct', {}).get('kpi_value') or 0
stock_close = kpis.get('stock_close_day_of', {}).get('kpi_value') or 0
sbc         = kpis.get('sbc_m', {}).get('kpi_value') or 0
gaap_oi_yoy = kpis.get('gaap_oi_yoy_growth_pct', {}).get('kpi_value') or 0
...
g_fy_rev  = next((g for g in guidance_rows if g['issued_for_period']=='FY26_Full' and g['metric']=='revenue_bn'), {})
...
eps_actual  = [e['eps_gaap_actual'] for e in eps_chart]
eps_est     = [e['eps_gaap_estimated'] for e in eps_chart]
...
def get_chartjs():
    ...
    try:
        ...
        with open(CHARTJS_CACHE, 'w') as f:
            f.write(js)
        print(f"OK ({len(js)//1024}KB cached to /tmp)")
        return js
...
<div class="page-header">
  <div class="page-header-inner">
    <div>
      ...
      <p>Fiscal period ending Jan 31, 2025 &nbsp;·&nbsp; Reported Feb 13, 2025 &nbsp;·&nbsp; <em>Everyone Is Wrong About AI, Including Me</em> — Digital FutureFest &#39;26</p>
    </div>
    <div class="header-meta">
      Generated {generated_at}<br>
      13 tables · 118 rows · /tmp/earnings_v2.db
...
<div class="section">
  <div class="section-header">
    ...
    <span class="tag">Reported Feb 13, 2025</span>
  </div>
  <div class="section-body">
    <div class="kpi-grid">
      ...
      <div class="kpi-card accent">
        ...
        <div class="kpi-note">~50% of non-GAAP &nbsp;·&nbsp; SBC ${sbc:.0f}M/qtr</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-label">Revenue YoY Growth</div>
        <div class="kpi-value pos">{sg(panw_q2, 'revenue_yoy_growth_pct', 'pct')}</div>
        <div class="kpi-note">Slowest in peer set (CRWD/ZS ~23%)</div>
      </div>
      <div class="kpi-card">
      ...
      <div class="kpi-card">
        ...
        <div class="kpi-value neg">{ah_chg:+.1f}%</div>
        <div class="kpi-note">Close ${stock_close} &nbsp;·&nbsp; Sell-the-news</div>
      </div>
      <div class="kpi-card accent">
        <div class="kpi-label">GAAP OI YoY Growth</div>
        <div class="kpi-value neu">+{gaap_oi_yoy:.0f}%</div>
        <div class="kpi-note">⚠️ Includes $175M litigation normalisation</div>
      </div>
    </div>
    <div class="trap">
      <strong>⚠️ The GAAP OI Trap</strong>
      GAAP operating income +{gaap_oi_yoy:.0f}% appears spectacular but embeds a $175.4M year-over-year litigation
      normalisation (Q2 FY25 had a one-time charge). Non-GAAP OI grew +13.5% — real but far more modest.
      SBC of ${sbc:.0f}M/qtr is the largest single reconciling item; GAAP EPS is roughly half of non-GAAP as a result.
      Always lead with non-GAAP for Street comparison. See the revenue chart below — the GAAP OI spike is visible.
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <div class="section-body">
    ...
    <div class="callout">
      <strong>Platformisation thesis</strong>
      NGS ARR +{ngs_arr_yoy:.0f}% YoY is the bull case metric: platform compounding faster than the revenue line.
      Deferred revenue ${defer_total:.2f}B + RPO ${rpo:.1f}B = ${defer_total + rpo:.1f}B+ of contracted visibility.
      FCF margin ~{fcf_margin:.0f}% signals the SaaS transition is not damaging cash conversion.
      Bear case (exchange #9): net new NGS ARR declined YoY ex-QRadar for two consecutive quarters.
    </div>
  </div>
</div>
...
<div class="section">
  <div class="section-header">
    <h2>Management Guidance — Issued Feb 13, 2025</h2>
  ...
  <div class="section-body">
    <div class="guidance-grid">
      ...
      <div class="guidance-panel">
        ...
        <div class="guidance-row">
          <span class="guidance-label">Revenue</span>
          <span class="guidance-value">${g_fy_rev.get('low_value', 0):.2f}–${g_fy_rev.get('high_value', 0):.2f}B</span>
        </div>
        <div class="guidance-row">
          <span class="guidance-label">Non-GAAP EPS</span>
    ...
    <div class="trap">
      <strong>⚠️ The Guidance Trap</strong>
      Q3 non-GAAP EPS ${g_q3_eps.get('low_value', 0):.2f}–${g_q3_eps.get('high_value', 0):.2f} is BELOW Q2 actual ${panw_q2.get('eps_nongaap', 0):.2f}.
      Sequential step-down ~5% signals margin pressure from newer SaaS products not yet at scale.
      This is the primary driver of the −3.5% AH reaction despite the beat. PANW has beaten guidance
      in 13 of 14 recent quarters — the question is whether this guide is conservative or genuine.
    </div>
  </div>
</div>
...
html += """      </tbody>
    ...
    <div class="callout">
      <strong>The peer read</strong>
      PANW has the slowest headline revenue growth (+14%) but the fastest NGS ARR growth (+37%) in the set.
      The thesis: ARR leads revenue by 4–6 quarters, so platform compounding shows up in ARR first.
      Bear case (exchange #9, Nowinski/Wells Fargo): strip out QRadar inorganic ARR, and organic net new ARR
      declined YoY for two consecutive quarters. Watch this metric in Q3 — it resolves the bull/bear debate.
    </div>
  </div>
</div>
...
for ins_row in insiders:
    ...
    html += f"""      <div class="signal-item">
        <div class="signal-header">
          ...
          <span class="badge badge-plan">{ins_row['insider_role']}</span>
          <span class="badge badge-est">${total:.1f}M sale</span>
          <span class="badge badge-neu">10b5-1</span>
        </div>
        <div class="signal-meta">{ins_row['transaction_date']} &nbsp;·&nbsp; {ins_row['shares']:,} shares @ ${ins_row['price_per_share']:.2f}</div>
        <div class="signal-note">{ins_row['notes'][:200]}{"…" if len(ins_row['notes']) > 200 else ""}</div>
      </div>
"""

html += """      <div class="callout">
        <strong>Analytical read</strong>
        All four transactions are pre-scheduled 10b5-1 plan sales — non-discretionary.
        CEO Arora's $143.7M block exercised options expiring Dec 2025 at a $33 strike, plan adopted March 2024 — 9+ months prior.
        President Jenkins sold at $203–208 six days after earnings (post-recovery). Mild constructive tell.
      </div>
    </div>
  </div>

  <div class="section">
    <div class="section-header"><h2>Sentiment &amp; Positioning Signals</h2><span class="tag">Feb 2025</span></div>
    <div class="section-body">
"""
...
html += """      <div class="info-box">
        <strong>Phase B note</strong>
        Historical Feb 2025 data not accessible via static web fetch (client-rendered pages).
        The dip-and-rip ($187.68 → $208.39 in 6 days) is the most reliable positioning proxy.
        For the June 2 print: capture live short interest + put/call via Claude in Chrome before data rolls.
      </div>
    </div>
  </div>
</div>
...
<div class="section">
  <div class="section-header">
    ...
    <span class="tag">Feb 13, 2025 Earnings Call</span>
  </div>
  <div class="section-body">
"""
...
// ── Horizon toggle ────────────────────────────────────────────────────────────
function setHz(hz, btn) {{
  var is90 = hz === '90d';
  document.getElementById('hz-90d-content').style.display = is90 ? 'block' : 'none';
  document.getElementById('hz-18m-content').style.display = is90 ? 'none'  : 'block';
  document.getElementById('hz-prelim-90d').style.display  = is90 ? 'grid'  : 'none';
  document.getElementById('hz-prelim-18m').style.display  = is90 ? 'none'  : 'grid';
  document.getElementById('hz-header').textContent = is90
    ? 'Horizon Read — 90-Day Trade'
    : 'Horizon Read — 18-Month Position';
  document.querySelectorAll('.hz-btn').forEach(el => el.classList.remove('active'));
  if (btn) btn.classList.add('active');
}}
    ...
    new Chart(document.getElementById('marginChart'), {{
      ...
      data: {{
        ...
        datasets: [
          {{
            label: 'GAAP',
            data: [{panw_q2.get('gross_margin_gaap_pct', 0)}, {panw_q2.get('operating_margin_gaap_pct', 0)}],
          ...
          {{
            label: 'Non-GAAP',
            data: [{panw_q2.get('gross_margin_nongaap_pct', 0)}, {panw_q2.get('operating_margin_nongaap_pct', 0)}],
  ...
  try {{
    new Chart(document.getElementById('epsChart'), {{
      ...
      options: {{
        ...
        plugins: {{
          legend: {{ position: 'top', labels: {{ boxWidth: 10, padding: 12, font: {{ size: 11 }} }} }},
          title: {{ display: true, text: 'GAAP EPS Surprise History (14 quarters)', color: C.muted, font: {{ size: 11 }} }}
        }},
        scales: {{
          x: {{ grid: {{ color: C.border }}, ticks: {{ font: {{ size: 10 }} }} }},
